...
class NumericalSolver:
    ...
    def calculate_game_cost(self) -> Rational:
        ...
        return sum(
            [self.payoff_matrix.row(i)[j]*a_mixed_strategy[i]*
             b_mixed_strategy[j]
    ...
    def _find_extremums_by_axis(
            self, axis: str, fn: Callable) -> List[Tuple[int, int]]:
        if axis == 'columns':
            lines = [self.payoff_matrix.col(j).T.tolist()[0]
                     for j in range(self.payoff_matrix.cols)]
        elif axis == 'rows':
            lines = [self.payoff_matrix.row(i).tolist()[0]
                     for i in range(self.payoff_matrix.rows)]
        else:
...
class BraunRobinsonTable:
    ...
    def solve(self, threshold: float = 0.01, max_steps: int = 2**64):
        while threshold < self.steps[-1].epsilon and \
    ...
    def make_step(self, a_strategy: int = None, b_strategy: int = None):
        if all([a_strategy is None, b_strategy is None]):
            ...
            raise Exception(
                ...
                ' Got strategies: %s', [a_strategy, b_strategy]
            )
    ...
    def get_a_mixed_strategy(self):
        ...
        return [Rational(counter[choice], len(self.steps))
                for choice in sorted(counter.keys())]
